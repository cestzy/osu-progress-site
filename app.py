...
def process_session_logic():
    ...
    try:
        # V5: Fetch 20 recent scores to ensure we catch everything
        ...
        new_feed_items = [] # V5: For Live Feed
        updates_made = False
        ...
        for score in reversed(recent_scores):
            ...
            acc = score['accuracy']
            raw_mods = score['mods']
            # V5: Strict FC Check (Perfect combo + No Miss)
            is_strict_fc = score['perfect'] 
            
            # V5: Mod Group Logic (Priority: DT > HR > HD > NM)
            mod_group = "NM"
            ...
            for goal in active_goals:
                ...
                # 2. Mod Check (V5 Feature: Strict Mod Matching)
                ...
                if req_type == 'pass':
                    ...
                    if is_strict_fc: success = True
                elif req_type == 'ss':
                    if score['rank'] in ['X', 'XH']: success = True
                elif req_type == 'count':
                     success = True
        ...
        # --- FETCH LIVE DATA FOR FRONTEND (V5) ---
        # 1. Stats
...
def home():
    ...
    try:
        ...
        if not user_row:
            cur.close(); conn.close(); session.clear(); return redirect('/')
        ...
        for row in cur.fetchall():
            ...
            formatted_goals.append({
                "id": row[0], "title": row[1], "current_count": current_prog, "count_needed": row[3],
                "criteria": row[4], "is_locked": row[5], "is_paused": row[6], "type": row[4].get('type', 'count').upper()
            })

        # V5: Fetch Star Counts (Strict FC Only)
